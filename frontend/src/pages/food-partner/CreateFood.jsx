...
const CreateFood = () => {
    ...
    useEffect(() => {
        ...
        const prevURL = videoURLRef.current;
        if (prevURL) {
        ...
        if (videoFile) {
            ...
            // Queue state update to avoid synchronous setState
            queueMicrotask(() => {
                setVideoURL(url);
            });
        } else {
    ...
    const onSubmit = async (e) => {
        ...
        if (!name.trim() || !videoFile) {
            setFileError("Name and video file are required");
            return;
        }
        ...
        try {
            ...
            if (window.toast) {
                window.toast.success("Food created successfully!");
            }
            navigate("/");
        } catch (error) {
            const errorMessage = error.response?.data?.message || "Failed to create food. Please try again.";
